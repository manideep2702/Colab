...
export const StudentDashboard: React.FC = () => {
    ...
    const topicMasteryData = [
        getModuleScore('python') || (submissions.length > 0 ? 75 : 0),
        getModuleScore('sql') || (submissions.length > 0 ? 60 : 0),
        getModuleScore('machine_learning') || (submissions.length > 0 ? 40 : 0),
        getModuleScore('data_analysis') || (submissions.length > 0 ? 55 : 0)
    ];
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            {recordings.map((recording) => (
                                <div
                                    key={recording.id}
                                    className="group relative rounded-xl overflow-hidden aspect-video bg-zinc-900 border border-white/10"
                                >
                                    ...
                                    <div className="absolute bottom-0 left-0 right-0 p-4 z-20">
                                        <Badge variant="default" size="sm" className="mb-2 bg-black/50 backdrop-blur-md border-white/10">{recording.module || 'Lecture'}</Badge>
                ...
                <div className="lg:col-span-4 space-y-6">
                    ...
                    <div className="p-6 rounded-3xl bg-[#0F0F11] border border-white/[0.05]">
                        ...
                        <div className="space-y-6">
                            ...
                            <ProgressItem label="Data Analysis" progress={topicMasteryData[3]} color="bg-rose-500" />
                        </div>
                    </div>
                    ...
                    <div className="p-6 rounded-3xl bg-gradient-to-br from-purple-900/20 to-indigo-900/20 border border-white/[0.05]">
                        ...
                        {codingSubmissions.length > 0 ? (
                            <div className="space-y-3">
                                {codingSubmissions.slice(0, 3).map(sub => {
                                    const challenge = codingChallenges.find(c => c.id === sub.challenge_id);
                                    const isPassed = sub.status === 'passed';
                                    return (
                                        <div key={sub.id} className="p-3 rounded-xl bg-black/20 border border-white/5">
